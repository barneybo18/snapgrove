    ...
    >
      <div className='relative cursor-pointer w-auto rounded-lg shadow-md overflow-hidden'>
        ...
        {isHoverred && (
          ...
          <div className={`w-8 h-8 rounded-md flex items-center justify-center border ${alreadySaved ? 'border-emerald-400' : 'border-gray-300'}`}
            onClick={() => saveToCollections(data?._id, user?.uid)}
          >
            <MdBookmarks className={`text-xl ${alreadySaved ?  'text-emerald-400' : 'text-gray-100'}`} />
          </div>
          </div>
          ...
          <div className="absolute inset-x-0 bottom-0 px-3 py-2 flex items-center gap-1 bg-gradient-to-bl from-[rgba(0,0,0,0.2)] to-[rgba(0,0,0,0.8)] backdrop-blur-sm flex-wrap">
            {data?.keywords.slice(0, 3).map((tag, i) => (
              ...
              {user?.uid === data?.users?._id && (
                <div className='absolute top-2 right-2 z-20 w-6 h-6 cursor-pointer rounded-full bg-[rgba(256,256,256,0.6)] flex items-center justify-center hover:bg-white' onClick={feedDelete}>
