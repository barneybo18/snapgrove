...
const CreatePost = () => {
  ...
  return (
    <div className='w-full h-auto flex items-center justify-start flex-col gap-4'>
      ...
      <input type="text" placeholder='Your post title here' className='w-full px-4 py-3 rounded-md border border-gray-200 shadow-inner text-lg text-primary font-semibold'
        value={title}
        onChange={(e) => setTitle(e.target.value)}
      />

      {/* categories  */}
      <Swiper grabCursor={true} spaceBetween={10} centeredSlides={false} className='mySwiper' slidesPerView={8}>
        {categoriesList && categoriesList.map(value => (
          <SwiperSlide key={value.id} className='py-4'>
            <div className={`px-2 py-1 flex items-center justify-center rounded-md border border-gray-200 hover:shadow-md shadow-inner ${category === value.name && "bg-gray-200"}`}
      ...
      <div className='w-full bg-gray-100 backdrop-blur-md h-370 rounded-md border-2 border-dotted border-gray-300 cursor-pointer flex items-center justify-center'>
        ...
        <>
          ...
          <> 
            ...
            {
              asset && ["video/mp4", "videos/avi", "video/mov", "video/wav"].includes(asset?.mimeType) && (
                <video 
                  src={asset?.url}
                  controls
                  className='h-full w-full object-cover'
      ...
      <div className='w-full flex flex-col gap-4 items-center justify-center'>
        <div className='w-full flex flex-col gap-4 items-center justify-center relative'>
          <input type="text" placeholder='type your tags seperated by comma' 
          value={keywords} onChange={(e) => setKeywords(e.target.value)} 
          className='w-full px-4 py-3 rounded-md border border-gray-200 shadow-inner text-lg text-primary font-semibold'
          onKeyUp={handleKeyUp}
          />
          <AiOutlineClear className='absolute right-3 text-xl text-primary cursor-pointer hover:text-2xl transition-all duration-150' 
            onClick={() => {
              setKeywords("");
              setTags([]);
            }}
        ...
        {tags.length > 0 && (
          <div className='w-full h-auto px-4 flex items-center justify-between flex-wrap border border-dashed rounded-md border-gray-200 gap-4'>
            {tags.map((tag, i) => (
              <div className='flex items-center justify-center gap-2 px-2 py-1 rounded-lg border border-gray-200 border-dashed shadow-inner hover:bg-gray-200 cursor-pointer' key={i}>
                <p>{tag}</p>
                <AiFillCloseCircle
      ...
      <textarea 
        ...
        className='w-full px-4 py-3 rounded-md border border-gray-300 shadow-inner text-lg text-primary font-semibold'
      />
